...
#         pass # Ignore network errors to keep loop running
...
#         print(f"Connection lost or timeout: {e}")
#         time.sleep(1) 
#         continue
    ...
    if not yolo_auto_mode:
        ...
        if cycle_elapsed >= 5.0:
            ...
            if obstacle_in_path:
                if not turn_delay_active:
                    send_bot_command('S', "OBSTACLE! BRAKING...")
                    cv2.putText(frame, "BRAKING...", (50, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
                    turn_delay_active = True
                    turn_start_time = current_time
                    pending_turn_cmd = 'L' if left_side_objects <= right_side_objects else 'R'
                    ...
                    if pending_turn_cmd == 'L':
                        send_bot_command('L', "TURN LEFT")
                        cv2.putText(frame, "MOVE LEFT", (50, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 165, 255), 3)
                    else:
                        send_bot_command('R', "TURN RIGHT")
                        cv2.putText(frame, "MOVE RIGHT", (50, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 165, 255), 3)
            else:
                turn_delay_active = False
                send_bot_command('F', "PATH CLEAR. MOVE FORWARD")
