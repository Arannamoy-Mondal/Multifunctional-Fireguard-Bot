...
<body onload="switchTab('M')">
  ...
  <script>
    ...
    function changeSpeed(step) {
      // --- New Code Start: Minimum speed changed from 150 to 50 ---
...
// Line Follower (Mode A) directly triggers these functions, so it uses the updated robotSpeed
...
void handleSpeed() {
  if (server.hasArg("val")) {
    ...
    // --- New Code Start: Backend minimum speed changed from 150 to 50 ---
...
void loop() {
  ...
  // Arduino sends F, L, R, S based on line sensors. ESP32 receives and moves motor at set robotSpeed.
