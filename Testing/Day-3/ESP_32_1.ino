...
<body onload="switchTab('M')">
  ...
  <script>
    ...
    function switchTab(mode) {
      ...
      // --- New Code Start: Handle YOLO UI Tab ---
      else if (mode === 'Y') {
    ...
    function changeSpeed(step) {
      // currentSpeed = Math.min(255, Math.max(100, currentSpeed + step)); // Bug/Wrong code: 100 is too low for motors
      // --- New Code Start: Set min speed to 150 to overcome DC motor deadzone ---
...
void handleCommand() {
  if (server.hasArg("cmd")) {
    ...
    // if (currentMode == 'M' || cmd == 'S') { // Bug/Wrong code: Blocks YOLO API commands
    // --- New Code Start: Allow YOLO mode to execute commands from Python ---
...
void handleSpeed() {
  if (server.hasArg("val")) {
    // robotSpeed = server.arg("val").toInt(); // Bug/Wrong code: Allows backend speed drop below 150
    // --- New Code Start: Enforce minimum PWM constraint in backend ---
