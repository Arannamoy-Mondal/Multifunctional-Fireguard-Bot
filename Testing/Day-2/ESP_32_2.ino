...
// --- Global Variables ---
int robotSpeed = 255; // Default max speed for better line following
char currentMode = 'M';
...
<head>
  ...
  <style>
    ...
    .tabs { display: flex; justify-content: center; gap: 5px; max-width: 340px; margin: 0 auto 15px auto; }
    .tab-btn { flex: 1; padding: 10px 5px; font-size: 12px; font-weight: bold; border: none; border-radius: 5px; cursor: pointer; color: white; background-color: #7f8c8d; }
...
<body onload="switchTab('M')">
  ...
  <div class="tabs">
    ...
    <button id="tabFire" class="tab-btn" onclick="switchTab('F')">FIRE MODE</button>
  </div>
...
void handleMode() {
  if (server.hasArg("val")) {
    ...
    stopMotors();
    Serial.print('M'); // Prefix for mode
    Serial.print(currentMode); // Send M, A, or F to Arduino
  }
  server.send(200, "text/plain", "OK");
}
void handleSpeed() {
