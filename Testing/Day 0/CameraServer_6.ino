...
// --- Global Variables ---
int robotSpeed = 200; 
char currentMode = 'M'; // Default Manual Mode

// --- Obstacle & Scanning Logic ---
int obstacleState = 0; // 0: Normal, 1: Stopping (10s), 2: Scanning (360)
...
// --- Motor Functions (ESP32 Core 3.x.x) ---
void setupMotors() {
...
void handleCommand() {
  if (server.hasArg("cmd")) {
    ...
    if (cmd == 'O') { obstacleState = 1; stateTimer = millis(); stopMotors(); }
    else if (cmd == 'C') { obstacleState = 0; stopMotors(); }
    else if (currentMode == 'M' || cmd == 'S') {
...
void handleMode() { if (server.hasArg("val")) { currentMode = server.arg("val")[0]; stopMotors(); obstacleState = 0; } server.send(200, "text/plain", "OK"); }
void handleSpeed() { if (server.hasArg("val")) { robotSpeed = server.arg("val").toInt(); } server.send(200, "text/plain", "OK"); }
...
void setup() {
  ...
  server.on("/action", handleCommand); server.on("/mode", handleMode); server.on("/speed", handleSpeed);
  server.begin();
}
...
void loop() {
  server.handleClient();
  if (obstacleState == 1) {
    if (millis() - stateTimer >= 10000) { obstacleState = 2; turnRight(); }
    else stopMotors();
  } 
  else if (obstacleState == 2) turnRight();
  else if (currentMode == 'A' && Serial.available()) {
